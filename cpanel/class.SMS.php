...
class SMS{
	...
	private function set_url($str){
		if(strlen($str)){$this->gate_way_url = $str;}
		else{die('invalid gateway');}
	}
	private function set_username($str){
		if(strlen($str)){$this->gate_way_username = $str;}
		else{die('invalid username');}
	}
	private function set_password($str){
		if(strlen($str)){$this->gate_way_password = $str;}
		else{die('invalid password');}
	}
	private function set_sender($str){
		if(strlen($str)){$this->sender_id = $str;}
		else{die('invalid sender id');}
	}
	private function set_message($str){
		if(strlen($str)){$this->message = $str;}
		else{die('invalid message');}
	}
	private function set_recipients($url){
		if(strlen($url)){$this->recipients = $url;}
		else{die('invalid recipients list');}
	}
