...
class StudentAdmin extends Admin{
	...
	public function createGroup($group_name, $group_members, $is_temp=0){
		if(strlen($group_name) and is_array($group_members) and sizeof($group_members)>1){
			$query = "insert into messenger_contacts_groups values(NULL,'".$this->getAdminID()."','".$group_name."','".implode(',',$group_members)."',".mktime().",".mktime().",".$is_temp.")";
			$link = AdminUtility::getDefaultDBConnection();
			$result = mysqli_query($link, $query);
			...
			AdminUtility::logMySQLError($link);
			return false;
		}else{die('invalid input for function createGroup(String name, String[] members)');}
	}
	...
	public function addToGroup($group_id, $new_members){
		...
		if(is_array($row) and is_array($new_members) and sizeof($new_members)>0){
			...
			foreach($new_members as $n){
				if( !in_array($n, $group_members)){$group_members[] = $n;}
			}
			$query = "update messenger_contacts_groups set group_members = '".implode(',',$group_members)."', modified = ".mktime()." where id=".$group_id;
			$link = AdminUtility::getDefaultDBConnection();
			$result = mysqli_query($link, $query);
			if($result){ return true;}
			//Log error
			AdminUtility::logMySQLError($link);
			return false;
		}else{die('invalid input for function addToGroup(Integer name, String[] new_members)');}
	}
	...
	public function removeFromGroup($group_id, $del_members){
		...
		if(is_array($row) and is_array($del_members) and sizeof($del_members)>0){
			...
			foreach($old_members as $n){
				if( !in_array($n, $del_members)){$new_members[] = $n;}
			}
			$query = "update messenger_contacts_groups set group_members = '".implode(',',$new_members)."', modified = ".mktime()." where id=".$group_id;
			$link = AdminUtility::getDefaultDBConnection();
			$result = mysqli_query($link, $query);
			if($result){ return true;}
			//Log error
			AdminUtility::logMySQLError($link);
			return false;
		}else{die('invalid input for function removeFromGroup(Integer name, String[] new_members)');}
	}
	
	public function renameGroup($group_id, $new_name){
			$query = "update messenger_contacts_groups set group_name = '".$new_name."', modified = ".mktime()." where id=".$group_id;
			$link = AdminUtility::getDefaultDBConnection();
			$result = mysqli_query($link, $query);
			if($result){ return true;}
			//Log error
